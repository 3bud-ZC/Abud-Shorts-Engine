...
RUN --mount=type=cache,id=pnpm,target=/pnpm/store bash -lc "set -euxo pipefail; pnpm install --prod --frozen-lockfile || true; pnpm approve-builds --all; pnpm install --prod --frozen-lockfile"
