...
export class WebhookService {
  ...
  public async createWebhook(url: string, events: WebhookEvent[] = []): Promise<WebhookSubscription> {
    ...
    if (this.db.enabled) {
      await this.db.query(
        ...
        [id, url, secret, JSON.stringify(events)],
      );
    }
  ...
  private async sendWebhook(hook: { id: string; url: string; secret: string }, event: WebhookEvent, data: any): Promise<void> {
    ...
    try {
      const res = await axios.post(hook.url, payload, {
        headers: {
          ...
          "User-Agent": "ABUD-Shorts-Engine-Webhooks/2.0",
        },
        timeout: 10000,
      });
    ...
    if (this.db.enabled) {
      await this.db.query(
        `INSERT INTO webhook_deliveries (webhook_id, event, payload, status, response_code, response_body, error, attempt_count, created_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, 1, now())`,
        [hook.id, event, JSON.stringify(payload), status, responseCode || null, responseBody || null, error || null],
      );
    }
  }
