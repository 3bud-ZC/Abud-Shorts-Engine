...
cmd_diagnostics() {
  ...
  if curl -fsS --max-time 60 "$url/api/v2/system/diagnostics/bundle" -o "$out" 2>/dev/null; then
    ...
    warn "The application could not be reached, so a reduced bundle was written instead."
    {
      ...
      echo "  \"containers\": {"
      echo "    \"app\": \"$(container_health abud-shorts-app)\","
      echo "    \"renderWorker\": \"$(container_health abud-shorts-render-worker)\","
      echo "    \"database\": \"$(container_health abud-shorts-postgres)\","
      echo "    \"automation\": \"$(container_health abud-shorts-n8n)\""
      echo "  }"
      echo "}"
    } > "$out"
